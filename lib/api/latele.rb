...
class API::Latele
  INDEX_URL  = 'http://www.latale.jp/community/com_imglist.aspx?flg=3&tag=0&page=%s'
  DETAIL_URL = 'http://www.latale.jp/community/com_imgview.aspx?seq=%s'
  ...
  def detail(sequence)
    ...
    begin
      ...
      published_at = lambda{
        strip_tags((detail/:td).fifth.inner_html).strip =~ /(\d{4}).(\d{1,2}).(\d{1,2}). (\d{1,2}):(\d{1,2})/
        DateTime.parse("#{$1}-#{$2}-#{$3} #{$4}:#{$5}:00 JST")
      }.call
      image_url = 'http://www.latale.jp/upload/upimage/' + sequence.to_s + 'n.jpg'
      comment = strip_tags((detail/:td)[6].inner_html).strip
    rescue
